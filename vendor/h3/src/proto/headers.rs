...
impl Pseudo {
    fn request(method: Method, uri: Uri, ext: Extensions) -> Self {
        ...
        // RFC 9114 section 4.4: ordinary CONNECT omits scheme and path.
        // Extended CONNECT retains both pseudo-headers.
        let plain_connect = method == Method::CONNECT && protocol.is_none();
        let scheme = if plain_connect {
            None
        } else {
            scheme.or(Some(Scheme::HTTPS))
        };
        let path = if plain_connect { None } else { Some(path) };
        let len = 1
