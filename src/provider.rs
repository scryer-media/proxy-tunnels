...
pub trait TunnelObserver: Send + Sync {
    ...
    /// Authorize an authenticated server before opening any forwarded channel.
    /// Hosts can enforce one trust decision across providers and revoke stale
    /// configuration revisions. Called again for every channel on a cached session.
    fn authorize_host_key(
        &self,
        _proxy_config_id: &str,
        _fingerprint: &str,
    ) -> Result<(), TunnelError> {
        Ok(())
    }
...
pub struct TunnelSpec {
    ...
    /// SSH username.
    pub username: String,
    /// Required OpenSSH-format Ed25519 private key.
