...
impl SshTunnelProvider {
    ...
    async fn connect(&self) -> Result<client::Handle<SshClientHandler>, TunnelError> {
        if self.spec.private_key_pem.trim().is_empty() {
            return Err(TunnelError::Configuration(
                "SSH tunnels require an Ed25519 private key; password authentication is no longer supported".to_string(),
            ));
        }
        let private_key = decode_ed25519_private_key(
            &self.spec.private_key_pem,
        ...
        let mut authenticated = false;
        {
            ...
            // not accept.
            let credential = russh::keys::PrivateKeyWithHashAlg::new(Arc::new(private_key), None);
            match handle
        ...
        self.authorize_host_key()?;

        // Trust on first use, after authentication succeeds. This persists
        // continuity of identity; it does not independently verify first use.
        let pin = {
    ...
    fn authorize_host_key(&self) -> Result<(), TunnelError> {
        let fingerprint = self
            .host_key
            .lock()
            .expect("host key state")
            .fingerprint
            .clone()
            .ok_or_else(|| TunnelError::Engine("SSH server has no verified host key".into()))?;
        self.observer
            .authorize_host_key(&self.spec.proxy_config_id, &fingerprint)
    }
    ...
    async fn open_channel(
        ...
        self.authorize_host_key()
            .map_err(ChannelFailure::Destination)?;
...
mod tests {
    ...
    #[tokio::test]
    async fn a_password_only_server_is_rejected_without_fallback() {
        let server = SshServerDouble::start(SshServerOptions {
            authorized_key_pem: None,
            ..SshServerOptions::default()
        })
        .await;
        let spec = crate::test_support::spec_for("proxy-key-only", &server.host(), server.port());

        let observer = Arc::new(RecordingObserver::default());
        let error = SshTunnelProvider::handshake(spec, observer.clone())
            .await
            .expect_err("a server accepting only passwords must fail");
        assert!(server.accepted_auth().is_empty());
        assert!(observer.pins.lock().expect("pins").is_empty());
        assert!(
    ...
    async fn a_passphrase_protected_ed25519_key_authenticates() {
        ...
        spec.private_key_pem = CLIENT_ED25519_PEM_WITH_PASSPHRASE.to_string();
