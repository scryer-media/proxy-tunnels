...
use crate::socks5::{MAX_REGISTRY_CONNECTIONS, Socks5Credentials, Socks5Front};
...
/// One address and its matching in-memory authentication credentials.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct TunnelFront {
    pub addr: SocketAddr,
    pub credentials: Socks5Credentials,
}
...
static SHARED: LazyLock<std::io::Result<TunnelRegistry>> = LazyLock::new(|| {
    ...
    tokio::runtime::Builder::new_multi_thread()
        ...
        .map(|runtime| TunnelRegistry {
            ...
            connections: Arc::new(tokio::sync::Semaphore::new(MAX_REGISTRY_CONNECTIONS)),
        })
});
...
impl TunnelRegistry {
        ...
        F: FnOnce() -> Result<Arc<dyn TunnelProvider>, TunnelError>,
    {
        ...
        if let Some(existing) = entries.get(proxy_config_id) {
            if existing.revision == revision && !existing.task.is_finished() {
                return Ok(existing.front.clone());
            }
    ...
    /// Stop only the revision whose trust decision was invalidated.
    pub fn stop_revision(&self, proxy_config_id: &str, revision: &str) {
        let mut entries = self.entries.lock().expect("tunnel registry lock poisoned");
        if entries
            .get(proxy_config_id)
            .is_some_and(|entry| entry.revision == revision)
            && let Some(entry) = entries.remove(proxy_config_id)
        {
            entry.stop();
        }
    }
...
mod tests {
    ...
    fn credentials() -> Socks5Credentials {
        static NEXT: AtomicUsize = AtomicUsize::new(0);
        Socks5Credentials::new(
            "test".into(),
            format!("secret-{}", NEXT.fetch_add(1, Ordering::Relaxed)),
        )
        .unwrap()
    }
    ...
    async fn the_same_revision_reuses_one_front_and_a_new_revision_restarts_it() {
        ...
        assert_ne!(first.credentials, restarted.credentials);
        assert!(!format!("{first:?}").contains(first.credentials.password()));
        assert_eq!(built.load(Ordering::SeqCst), 2);

        // The old front is gone.
        tokio::task::yield_now().await;
        assert!(
            connection_is_dead(first.addr).await,
    ...
    async fn a_front_binds_loopback_on_an_ephemeral_port() {
        ...
        let addr = registry
            ...
            .expect("start");
        assert!(addr.addr.ip().is_loopback(), "{addr:?}");
        assert_ne!(addr.addr.port(), 0);
        assert_eq!(registry.front_addr("proxy-loopback"), Some(addr.addr));
    }
    ...
    async fn stopping_a_tunnel_closes_its_front() {
        ...
        tokio::task::yield_now().await;
        assert!(
            connection_is_dead(addr.addr).await,
            "front still serving {addr:?}"
        );
    }
